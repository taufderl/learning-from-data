...
#
#   This version is based on single characters.
#
import math
import pickle
import os
...
FOLDER = './data/'
TRAINED_DATA_FILE_DUTCH = FOLDER + 'trained_data_dutch.pk'
TRAINED_DATA_FILE_OTHER = FOLDER + 'trained_data_other.pk'
TRAINED_DATA_FILE_PROPERTIES = FOLDER + 'trained_data_properties.pk'
...
class DutchLanguageRecognizer:
  ...
  def train(self, dutch_tweets, other_tweets):
    ...
    for line in dutch_tweets:
      for token in line:
        ...
        if token in tokens:
          ...
          tokens[token] = 2 # initially added one to set one if 0 occurencys

    # calculate probability for each token
    dutch_data = {}
    for token in tokens:
      dutch_data[token] = (tokens[token]/(total_tokens+vocabulary))

    # calculate probability for unknown tokens
    dutch_data['UNKNOWN_TOKEN'] = 1/(total_tokens+vocabulary)
    ...
    other_data = {}
    for token in tokens:
      other_data[token] = (tokens[token]/(total_tokens+vocabulary))

    # calculate probability for unknown tokens
    other_data['UNKNOWN_TOKEN'] = 1/(total_tokens+vocabulary)
  ...
  def test(self, tweets):
    ...
    for tweet in tweets:
      ...
      for token in tweet:
        if token in dutch_prob:
          score['DUTCH'] += -math.log(dutch_prob[token])
        else:
          score['DUTCH'] += -math.log(dutch_prob['UNKNOWN_TOKEN'])
        if token in other_prob:
          score['OTHER'] += -math.log(other_prob[token])
        else:
